...
## Pendiente (post-integración)

- **Merge `main` → `FRONT`:** backend Supabase progreso/entrevista (PR #21) aún no en rama `FRONT`
- Persistencia Supabase en producción (migraciones 012–015)
- Preguntas entrevista: UI usa pool por skill (`MOCK_QUESTIONS`); API usa templates genéricos — alinear post-pitch
- “Ver feedback” en historial: solo último resultado en store (mejorar con endpoint detalle)
